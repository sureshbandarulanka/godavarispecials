...
import { getAdminBanners, deleteBanner, toggleBannerStatus, Banner } from '@/services/bannerService';
              ...
              {banners
                ...
                .map((banner) => (
                  ...
                  <div className="admin-item-actions">
                    ...
                    <button 
                      className="btn-icon" 
                      onClick={(e) => handleToggleStatus(e, banner.id, banner.isActive)}
                    ...
                    <button 
                      ...
                      onClick={(e) => handleDelete(e, banner.id, banner.imageUrl)}
                      disabled={actionLoading === banner.id}
                      title="Delete"
                    >
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div className="admin-item-content">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      ...
                      <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600 }}>PRIORITY: {banner.priority}</span>
                    </div>

                    <h3 className="admin-item-title">{banner.type?.toUpperCase() || 'GENERAL'} BANNER</h3>
                    <p className="admin-item-subtitle">{banner.tagline || 'No tagline set for this banner'}</p>
                    
                    {/* Scheduling placeholder structure */}
                    <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: '12px' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '10px', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 700 }}>Schedule</div>
                        <div style={{ fontSize: '12px', color: '#64748b' }}>Always active</div>
                      </div>
                    </div>
                  </div>
                </div>
